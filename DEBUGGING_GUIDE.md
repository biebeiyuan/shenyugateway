...
| 沈予一次并行叫多个工具就必定 400 `TOOL_USE_RESULT_MISMATCH`，只叫一个永远正常，看着像上游偶发 | `shenyu_gateway/upstream_adapter.py::_openai_to_anthropic`：`role == "tool"` 每个结果各自 append 一条 user 消息，而 Anthropic 要求一条 assistant 里的 N 个 `tool_use` 由紧跟着的**同一条** user 消息装齐全部 N 个 `tool_result`；N=1 时 1+1 恰好合法，所以缺陷只在并行时露头 | 别停在"概率问题"：把日志按「那一轮几个工具」切开数，单工具全过、并行全炸就不是概率。再看请求日志 Upstream 标签里 assistant 之后是几条 user——一个结果一条就是它。"以前并行也正常"通常是记错了形状：工具走 `shenyu_gateway_tool` 总机时一次只派一个，是串行 |
| 无参工具（`{}` 参数）回传给 OpenAI-compatible 上游时 400 `Improperly formed request.`，同一个工具走 Anthropic 上游却正常 | `shenyu_gateway/upstream_adapter.py`、`shenyu_gateway/streaming.py::_ensure_stream_tool_call`：Anthropic 流对无参工具不发 `input_json_delta`，累积出来的 `arguments` 是空字符串；Anthropic 出口被 `_coerce_json_object` 兜成 `{}`，OpenAI-compatible 出口原样透传，上游解析不了 | 同一个值有多条协议出口时，先比对两条出口的实际产物再定位（`_sanitize_openai_compatible_messages` 与 `_openai_to_anthropic` 各跑一遍同一条消息）。只有一条路有兜底正是症状按上游分裂的原因；修在产生处或所有出口的共同入口，别只堵报错那一条 |
| 400 `TOOL_USE_RESULT_MISMATCH` 但结果一个没少、并行合并也确实生效了——同一个错误码的第二个成因 | `shenyu_gateway/context_layers.py::_slide_off_tool_transcript` 挡住的那种：记忆岛的锚点偏移按「非 system 消息条数」算，不认工具轮的边界，落在某条 `role: tool` 上就等于在 `tool_use` 和 `tool_result` 之间插了一条消息（岛在 Anthropic 出口渲染成 `role: user`）。锚点在 epoch 未重置时是上一轮的数字原样带过来的，窗口头部滑动后同一个数字指向的已不是同一条消息 | 先搜网关日志里的 `[ToolContract]`——`shenyu_gateway/request_logs.py::_tool_result_contract_mismatches` 会在 payload 发出前数一遍（两种协议形状都数：Anthropic 看紧跟着那条 user，OpenAI 看后面连续的 `role: tool`），缺口连 id 一起 warning 出来，同一份也落在请求日志 Upstream 标签的 `tool_result_contract_mismatch` 里。有 `missing_ids` 但结果总数没少就是被什么东西夹开了；`unexpected_ids` 有值是孤儿结果（调用被谁丢掉了）。再对 `memory_island_insert_index` 和那一轮工具的下标：落在两者之间就是它。这条也覆盖历史本来就残缺的情况（客户端只回了 N 个里的 M 个），那种合并救不了 |
