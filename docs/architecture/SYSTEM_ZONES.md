...
| 盼圃 | 果子不过期、不删、不催：没有预计日期的一直挂着等，过了日期的也只是继续挂着。这面墙没有任何自动注入路径，不调工具它就不在提示词里。摘下那天定格的园况和摘果感言永久冻结，读的时候不重算；纸条只追加，摘的时候连纸条一起留下。天气只读圆圆所在地实况、只记会留下后果的那几类，取不到天气绝不能让盼圃打不开 | `README.md` § 盼圃 (Orchard) |
...
| Room 模式（`room_tools`、`room_notebook` 门、窗边报纸） | **Room 专属分支**：PWA 的 `+` 入口发送完整时间戳消息触发，不是独立客户端 | `private_capture.py` 检测、`prepare_messages.py` room 分支 |
