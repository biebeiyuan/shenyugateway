...
- **Priority inside a shared total, not extra places.** The Mem lane is capped at `MEM_NOTE_LIMIT` (default 3) notes in total, and reminders take those places ahead of ordinary recall — carried-over hung reminders first, then newly-due ones, then recall (`context_builder.py::_merge_due_reminders`). The lane is not "recall plus reminders": the island is a prompt-cache breakpoint anchor, so an uncapped lane means an uncapped cacheable prefix. `DUE_REMINDER_MAX` (3) additionally caps how many the query itself returns, and does not count reminders already hung on the island (those are stamped, so the `reminded_at is.null` filter skips them).
