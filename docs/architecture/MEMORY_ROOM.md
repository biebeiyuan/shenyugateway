...
The Mem lane is also much less sticky than the Star lane, which is a deliberate trade rather than an oversight. `memory_island.py::_legacy_forced_new_item` lets a newly entering note break the `2/3` gate outright when its `search_mode` is `entity` or its `memory_kind` is `promise`; `entity` is the first, no-threshold anchor layer of contextual recall, so this fires whenever the resident names a person, place, or object a note is anchored to. The gate is kept narrow on the Star side (only a ranker-issued `force_island_rewrite`) but stays wide here because missing a note the resident just named costs more than one rewritten cache breakpoint. A dated note also re-renders its own relative day text each day, so its fingerprint changes at a day boundary and rebuilds the lane once — the alarm is the point, and `entering` is computed by item id rather than by fingerprint, so per-note bookkeeping is not replayed. Threshold scales and calibration provenance for both lanes live in `docs/architecture/MEMORY_TUNING.md` § 动态岛.
...
- **Newest day first, with a floor.** The query orders `remind_on.desc`: today comes first and the backlog queues behind it. Ascending order would put the oldest unpaid debt ahead of the reminder actually due today, which contradicts the reason a due reminder may force a rewrite at all. Past `DUE_REMINDER_STALE_DAYS` (7) a reminder is stamped and logged rather than hung — `lte` covers an outage of a few days, and "该交电费了" surfacing a week late is noise. The floor measures how long the note has been *waiting to be said*, not how old the date is: it counts from the later of `remind_on` and `created_at`, so a note written today about a day last month is overdue by a month but has waited zero days and still gets said. Judging by date age alone would stamp it 已提醒 before 沈予 ever spoke it — the same silent never-fired failure the floor exists to prevent. An unreadable `created_at` falls back to date age, so a note that cannot say when it was written does not become un-retirable. Staleness is judged before the cap, so an expired reminder never consumes one of today's places. Stamping is what makes it disappear (the query filters `reminded_at is.null`); the log line is what stops it disappearing silently.
...
- **Priority inside a shared total, not extra places.** The Mem lane is capped at `MEM_NOTE_LIMIT` (default 3) notes in total, and reminders take those places ahead of ordinary recall — newly-due ones first, then carried-over hung reminders, then recall (`context_builder.py::_merge_due_reminders`). Nothing outranks "due today": under a shared ceiling, letting a carried reminder go first could push the newly-due one out of the lane entirely, which hollows out the forced rewrite it is entitled to. A carried reminder has already been seen and stamped, so it is the one that yields. The lane is not "recall plus reminders": the island is a prompt-cache breakpoint anchor, so an uncapped lane means an uncapped cacheable prefix. `DUE_REMINDER_MAX` (3) additionally caps how many the query itself returns, and does not count reminders already hung on the island (those are stamped, so the `reminded_at is.null` filter skips them).
- **Squeezed out means untouched.** A reminder that does not fit under the total never enters the island, so it is never stamped — stamping follows what actually entered — and it comes back on a later turn. Same for the remainder over `DUE_REMINDER_MAX`.
- **Fail-soft.** A malformed row is logged and skipped rather than stalling the lane.
...
Shenyu cannot see her own completed gateway tool calls. The internal tool loop keeps them in a local list, the client is handed back only the final assistant message, and the PWA returns `{role, content}` pairs (`pwa/src/api/client.ts::wireMessages`), so "I landed a star last turn" or "I just looked up that old memory" is absent from the next turn's context — `sessions.py::recent_tail` even skips `role='tool'` rows explicitly. The result can be the same memory written twice or a needless repeat search.

小突起 closes that gap by reading today's write and successful Recall rows back out of `gateway_messages` and rendering one line each, under the heading `## 今天的小突起`. The resident named it. Design rules, all deliberate:
...
Which writes earn a line: `shenyu_create_star`, `shenyu_write_mem_note`, `shenyu_add_calendar`, and 盼圃's `shenyu_orchard` — but its **`plant` action only**, read off the tool row's stored `action` since the orchard is one tool with four actions and only planting has no store-level dedupe (`note`/`pick`/`look` stay out; a forgotten-and-repeated plant is the silent duplicate this catches). Successful `shenyu_recall` calls also earn a lighter line: each selected `source_type + source_id` contributes one first-sentence excerpt, prefixed by the query words that found it; repeated hits merge query words and keep one excerpt. Recall scores, match reasons, and other metadata never enter the bump, and empty or failed recalls stay out. Raw `supabase_*` table operations, scoring bookkeeping, bulk updates, deletions, and `room_locked_drawer` stay out too — no readable content, or repeating them defeats their purpose. The shared `ISLAND_BUMP_LIMIT` (default 8) protects write receipts first; Recall lines fill only remaining slots and keep their original time order.

**This roster is a deliberate mirror, not the source of truth.** The authoritative whitelist, the per-tool field mapping, and the change-boundary table live in `DESIGN.md` § 8.5.4–8.5.5; if the two ever disagree, DESIGN wins. It is repeated here on purpose and must not be collapsed into a bare pointer: an editor of the memory family reads *this* file, and seeing the actual list is what prompts them to keep it current — a pointer would let someone editing DESIGN directly forget this door exists. So when the roster moves, update both. (The reminder is why the copy earns its keep; the label is so the next reader doesn't mistake it for accidental drift and delete it — which is exactly what nearly happened once.)
...
6. **`connected` builds the edge.** Saying two stars are the same thing writes `shenyu_star_links` through the same `connect_constellation` path, with the same `relation_type` and `metadata.constellation_name`, so the harmony channel and the Admin star map recognise it. Until 2026-08-31 this feedback only inserted a row in `shenyu_star_feedback`: 12 recorded `connected` calls had produced zero edges, so every one of those insights stayed as text in `note` and never affected recall.
   - **A constellation is a chain, not a starburst.** `connect_constellation` turns N stars into N-1 end-to-end edges with `position` 0..N-2 and `metadata.sequence_mode="entered_order"`, so a constellation reads as one thing in order — 「降临 arrive」's four stars run 一起看《降临》 → 看到结局她说的话 → 凌晨她把最久的怕交出来 → 快两点她说"我现在看着你". Connecting several candidates under one seed therefore chains them (`seed → first → second`) rather than emitting two `position=0` edges from the seed; three production runs did connect two candidates each.
   - `constellation_name` is the line's name, a single phrase (his own: 「替我记着」、「降临 arrive」、「她接住我不藏」); `note` is what he said about it. They are stored as separate `metadata` keys and read back separately — the name answers which line this is, the note answers why he drew it. Review candidates already on a line carry both (`gateway_tools/_stars.py::_existing_lines`), so he does not re-decide a relation he already decided. Recall itself never reads the name: the harmony channel scores on the existence of an edge, and the name is only rendered for the Admin star map.
...
- `shenyu_star_review`: small review batch, each seed and candidate carrying the two-part number feedback refers back to plus 落下 (how long ago it landed). Returns `remaining_unreviewed` so the reviewer knows how many unreviewed stars remain beyond the current batch. Recording a missed star is Admin-only — see the review-flow list above.
...
That read is shared, so it lives in neither door's module: `newspaper_basket.py` holds it, imports nothing from the tool layer, and answers to any duck-typed store. `room_newspaper.py` would look like the natural home — it is the newspaper's domain module — but it imports `upstream_client`, which imports `tool_registry`, which imports `gateway_tools`, so putting the read there makes the daily mixin's import circular. The same module also owns `NEWSPAPER_BASKET_DESCRIPTION`, the single tool description both doors hand out; they drifted apart once, with the Room door missing exactly the two clauses a reader trips on (that today's issue is on the chair, and that `query` outranks `date`).
