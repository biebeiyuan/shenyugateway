...
The build stamps `sw.js` with the same build ID and a complete shell/hashed-asset manifest. Installation verifies the live build before and after precaching; an incomplete/mismatched candidate is discarded without deleting the preceding cache. Updates do not call `skipWaiting` or `clients.claim`, and the page has no controllerchange reload. Navigation falls back to the paired canonical shell on network errors/non-success status or after 1.2 seconds when a shell exists, including query-string handoffs. API/SSE and build proof bypass shell caches. Old assets are pruned only during activation when no chat pages are present. An initial never-cached visit still requires the network.
