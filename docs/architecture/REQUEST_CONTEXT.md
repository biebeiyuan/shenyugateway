...
The concrete JSON paths differ between Anthropic and OpenAI-compatible payloads, but both adapters use the same logical boundaries. If the island changes, the island-end breakpoint is rewritten while the prefix before the island can still hit.

The `slow` (calendar) and `heartbeat` layers share the `system.end` breakpoint, so a fresh heartbeat batch or a new calendar page would otherwise rewrite the whole system prefix every time. `system_prefix_buffer.py::resolve_system_prefix` holds those two layers behind a buffer gate (`prepare_messages.py`, between rendering the layers and assembling them): it keeps the previously served `slow`/`heartbeat` text — stored as `system_prefix_state` alongside `island_state` in `context_window_states` — until either the buffer window elapses or the epoch resets on a trim/branch (a rebuild is due anyway). The buffer uses the current upstream protocol's TTL (`ANTHROPIC_CACHE_TTL` for Anthropic, `OPENAI_CACHE_TTL` for OpenAI-compatible), so a new heartbeat or calendar page is held for that protocol's cache lifetime. Removed content follows the same hold rule; it disappears when the buffer expires or an epoch rebuild applies the new snapshot. If the TTL cannot be parsed, `buffer_seconds` is 0 and the gate fails open to today's behaviour (refresh every turn) rather than pinning stale text. This is a passive-injection concern only: 沈予 reading her own heartbeats via `shenyu_read_heartbeat` hits the live table and is never buffered. The decision (`held`/`refreshed` with a reason) rides the content-free `context_window_events` log; Admin's session detail exposes `system_prefix_refreshed_at` so an injected-but-still-buffered heartbeat shows as 缓冲中.

The `context_window_state` returned by `select_chunked_window` is the cross-round handoff. It must preserve every layer state that a later preparation step consumes, currently `island_state` and `system_prefix_state`, alongside the epoch and trim metadata. A new stateful layer must be added to both the persistent store shape and this return-state passthrough; testing only the store round-trip does not cover this handoff.

小突起 (the island bumps: one-line receipts for the memory writes Shenyu already made this waking day, plus lightweight crumbs for successful Recall) rides immediately after `memory_island.end` and deliberately carries no breakpoint of its own. It travels as the `MEMORY_ISLAND_BUMP_KEY` side field on the island message rather than inside the island text, because the island text is hashed to decide its version — folding the receipts in would rewrite the island on every star, mem note, or calendar write. Anthropic renders it as a second text block after the cached island block; OpenAI-compatible renders it as its own system message after the island message. Either way it sits outside the island's cached prefix but inside the stretch before the tail breakpoint, so a changed receipt costs a tail rebuild and never an island rewrite. It follows the island anchor automatically, so changing `ISLAND_TAIL_MESSAGES` moves both together. For pure text, the final breakpoint stays on the current user's last content block. When the retained client window contains Operit extra-bundle attachments, it moves to the user turn immediately before the newest three user turns; image-only windows use the turn before the newest two. Those recent turns remain fully visible to the model but stay outside the stable cached prefix because attachment and image retention can rewrite them on later requests. Gateway tool-loop continuations still mark the latest tool result instead of applying the user-turn guard.
...
`ISLAND_TAIL_MESSAGES` (default 32, range 8–80, editable in Admin under 节奏与窗口) is how many messages the island hangs before the end of the retained history. It counts messages, not human turns — a turn is usually two messages. A new value applies only when the epoch resets (a high-water trim or a branch): inside an epoch the stored `island_anchor_offset` is reused, because moving the anchor mid-epoch would relocate the cacheable prefix and discard the whole window's cache on a settings change. Changing it therefore looks like nothing happened until the next boundary, which is intended.
...
The in-memory admin request log keeps a bounded display snapshot of the Memory Island selected for each request so Stars and Mem changes remain inspectable without enabling full payload retention. Star text is capped at 220 characters and Mem text at 180 characters per item. The rendered 小突起 lines follow the same payload-retention switch as the island's `rendered_text`, and appear in Admin as their own block rather than a third island lane: the two island lanes describe what is currently on the island, while the bumps describe what was written today. This snapshot follows the existing recent-log capacity and disappears on process restart; SQLite window/island decision records remain content-free.
...
History-event classification removes transient images and Operit extra bundles before comparing raw request windows. Text-only content is canonicalized semantically, so a plain string and an equivalent list of text blocks are the same lineage. Image expiry or client content-shape flattening must not create `history_branch`.

Expired-photo placeholders are part of that rule, and two different mechanisms protect them at two different stages — do not conflate them.

A photo the PWA has evicted locally arrives as an `EXPIRED_IMAGE_MARKER` block (`client_extra.py`). Classification runs on the raw request window, *before* the image trim, so this is the block it actually compares; it normalizes away because both image-block detectors accept the shape, exactly like the original image. That detector — not any text matching — is what keeps local photo expiry from looking like an edit.

The placeholder's replacement **text** matters one stage later. Snapshots are written after the trim, so a snapshot stores whatever replaced the image — and when 沈予 had saved that photo to his album, that is his own words, which differ per photo. The PWA re-feeds snapshots as history on thread handoff (`pwa/src/session/history.ts`), and then classification does see those sentences. So normalization matches the fixed **prefix**, never the whole sentence: matching the sentence would make two handoffs carrying different notes look like different histories, classify as `branch`, and discard the epoch's cache. Anything that rewrites this placeholder must keep the prefix. `tests/test_gateway_trim.py` asserts both halves — that two different notes normalize identically, and that a marker block normalizes like the image it replaced. A rolling-window head slide — the current window equals the previous window minus its oldest messages, plus optional appended tail turns, with at least 8 overlapping messages — is normal bounded-client behavior (e.g. the PWA reloading after its localStorage cap, or re-entering a thread from a gateway snapshot): it classifies as the matching append/retry event, keeps the epoch, and shifts the stored gateway window start by the slide so the retained upstream window is unchanged. A branch reset is reserved for a real edit before the active tail turn.
...
New runtime tool receipts retain `reply_version_id` and the real `tool_call_id`, including results reused by the existing argument cache without re-execution. Hydration matches known reply identities and merges missing call phases without moving locally observed offsets or replacing completed results. Only a recorded boolean `tool_ok` may turn unknown `ok:null` into a known status; payload text or JSON never proves execution success. Missing status renders as “结果已返回”, not “完成”; `false`, zero offsets, empty result strings and null inputs remain meaningful observed values. Legacy null outputs and empty identity labels alone may be repaired. An unchanged merge returns its original array, so repeat hydration is a no-op without property-order-sensitive serialization. Equal reply text does not suppress missing-tool recovery. Only unique identity-less legacy text can use fallback association; ambiguous groups remain unassigned. A failed legacy localStorage write preserves the prior committed value and reports failure; it never strips tool outputs/events to claim success.
...
Ordinary `openSession` is device-first. The target gateway/session manifest is loaded before its background refresh; only a session with no local record uses the snapshot-first selector. A known active tail may append an exact, continuous server suffix. Server omissions, shorter windows, network errors and different roll branches are never deletion instructions. Clean cold-start recovery remains an explicit same-session alternative, not an automatic source to merge.

`transcriptStore.ts` owns `shenyu-pwa-transcripts-v1`: per-normalized-gateway/session manifests and metadata-only message rows commit in one strict-durability IndexedDB transaction. Full tool outputs, thinking/echo segments, selected roll state, drafts and reading anchors are retained; transient image URLs/bytes remain outside this store. Saves update changed rows and the active ordering together, without the old 240-row limit. Superseded branch rows stay out of the active request, but remain retained. Revision compare-and-swap rejects stale tabs rather than overwriting a newer committed copy. This is device persistence, not a cross-device archive or protection against explicit browser-data deletion.

The old single localStorage slot is bound exactly once to its original stored gateway/session, including an empty-source marker. Nonempty legacy data with no session owner is not guessed. Its exact source is preserved before import, the durable copy is read back before migration completion, and future opens never import the same legacy bytes under a different last-opened session. Unknown row metadata is carried forward. Corrupt/unsupported records or failed writes are visible, preserve the previous committed state and never trigger an empty-record replacement. The legacy localStorage writer remains compatibility-only; App saves through `useTranscript.ts`.

A write conflict has an explicit **保留本页并重新同步** action that does not call the failing ordinary save first. `useTranscript.ts` invalidates pre-resync queued snapshots and blocks editing/navigation during the operation. It commits a full local recovery copy before reading the latest manifest, retains that saved original too, then CAS-commits a conservative merge before applying and unlocking. A read alone never unlocks a stale writer. Healthy pages cannot enter conflict recovery, and draft retrieval during conflict explains that resynchronization must come first. Scrolling is not an edit: the content guard excludes the viewport and applies the latest reading position. Failed checkpoints, changed live input, changed session, missing/corrupt records, or a second concurrent write preserve the live page and fail closed; no blind revision retry is allowed. Only exact known replies with identical body/echo can contribute missing process data; divergent drafts and reply branches stay in their own copies rather than being spliced into the next model request. When the saved composer has no draft, attachments or edit target, an ordinary local draft and its attachment metadata stay in the composer; edit-mode drafts are never transplanted across branches.

Recovery copies are immutable `add` records under typed array keys in the existing `lists` store, separate from replaceable session-list string keys. This additive format needs no database-version upgrade that could block another open tab. They are scoped to gateway/session, survive reopening, and are never pruned as list caches. Auxiliary copies have admission limits of 20 copies and 32 MiB of serialized UTF-8 data per scope (not a claim about browser physical allocation). Same-kind identical content, excluding reading position, reuses an existing immutable copy. Count/byte checks and insertion share a transaction, so concurrent tabs cannot both take the last slot. Hitting a limit rejects a new copy without deleting existing ones or limiting ordinary chat history; older over-budget copies are preserved. Settings offers explicit single-copy removal after a warning to export unique contents first. That operation touches only the exact scoped recovery key, never the active manifest, retained messages, legacy backup or list cache. Settings lists both originals, previews their drafts and exports full messages/versions/process data. **找回这份草稿** checkpoints the current input first, then CAS-commits the selected draft and attachment metadata with `editId=null`; it never sends a message or replaces the active conversation. Ordinary photo-byte retention is unchanged. The visible main record follows the latest saved branch; keeping both originals is not a claim that divergent branches can be automatically combined.

`useTranscript.ts` serializes captured save jobs per session. A send commits the user turn and new reply identity before POST; tool frames and one-second streaming checkpoints submit progress, drafts/scroll use a short debounce, and pagehide is supplemental only. Receipt completion is awaited before releasing the send lock. Server merge, storage load and session navigation carry an operation generation so a stale response cannot select a session, clear a newer draft or overwrite its manifest. The first local paint/reading position does not wait for config/models/session-list requests. Last successful visible/hidden lists are cached independently from transcript revisions.

The build stamps `sw.js` with the same build ID and a complete shell/hashed-asset manifest. Installation verifies the live build before and after precaching; an incomplete/mismatched candidate is discarded without deleting the preceding cache. Updates do not call `skipWaiting` or `clients.claim`, and the page has no controllerchange reload. Navigation falls back to the paired canonical shell on network errors/non-success status or after 1.2 seconds when a shell exists, including query-string handoffs. API/SSE and build proof bypass shell caches. Old assets are pruned only during activation when no chat pages are present; that guard controls cleanup, not whether the worker itself activates. An initial never-cached visit still requires the network. Online navigation can return newer HTML while the controlling worker and its offline cache are still old.

`usePwaUpdates.ts` separately observes the running page build, the controlling/active worker build, installing state and a waiting candidate. The worker answers only the read-only `SHENYU_PWA_BUILD_INFO` message; an old worker that does not answer within one second is unknown, never assumed current. Settings distinguishes downloading, downloaded/waiting, unknown, and old offline build states. Deployment proof is fetched from the hosting page origin (not a separately configured chat gateway), without leaking that gateway's token. Update checks do not reload/activate/clear anything. Close all old chat pages only after preserving unsaved content; switching a phone app to background may leave its client alive. A newer online page is not proof its offline build switched. The installed-device old-build → background/multiple pages → new-build → online/offline reopening path remains separate from Chromium verification; never clear site data to make the test pass.

`pwa/src/session/reconcile.ts` has two entrances: `applyReplyRecovery` reads `/reply-recovery`; `applyReconciledTail` reads `recent_messages`. The optional `reply_version_id` query asks for that exact recorded reply within the existing bounded inspection window, even after another turn; without it the legacy endpoint remains latest-request-only. A missing requested ID never falls back to the latest reply. Known local IDs must match; the fallback can check local `archiveEvent.id` against `source_id` without replacing its timestamp. Only unversioned legacy recovery may use a text anchor. Accepted completion/identity/replay/tool-only changes must persist even when body text is unchanged. An unfinished tool start also keeps tail recovery eligible; foreground/startup retries use the same backoff as a broken stream. Partial endpoint failures do not discard a successful independent result; reads have a 12-second deadline and obsolete recovery work is aborted.
...
- New PWA opts in with `X-Shenyu-Album-Photos: true`. Explicit `shenyu_album_send` commits `album_message_media` before reporting success and emits an existing `shenyu.tool_event` with `photo` and `reply_version_id`. The same fields travel in non-streaming tool events. A client without this capability or a valid reply envelope receives a clear failure, never a pretend share. The per-reply nine-photo limit and share-ID conflicts return their actual validation reason; storage exceptions remain failures. `open`/`send` bypass generic argument-result caching, including broker aliases: each new call keeps its effect, while replay of the same share ID is deduplicated by the ledger. Generic cached failures retain `ok: false` and their error classification. Broker start/end events resolve aliases through the same registry as execution, for both success and failure; the raw wire name and input remain available. The normal error decorator supplies the existing `ps` hint, and opted-in event output is the same JSON the model receives. PWA renders blocked album actions without claiming an image was opened or sent; failed events never create photo attachments.
- A user message's `media` is whitelisted metadata bound to its `archive_event.id`; incoming photo IDs and descriptions are discarded. PWA's per-attachment `image_index` counts both real images and expired fingerprint blocks, but is null when an attachment has neither bytes nor a fingerprint. The backend computes fingerprints where pixels are present, then drops the slot helper. The nine-item metadata limit applies before both in-memory use and storage, including malformed slots and failed write/readback paths. Metadata-only restored rows may still resolve an album photo by their supplied byte fingerprint without a current image block: this is an authenticated owner's recovery hint, not proof of upload or an authorization credential. No ordinary-image BLOB is created. Incoming assistant `media` cannot authorize a share: only the server ledger supplies share references.
- `media` is gateway-only, retained in snapshots and removed before provider calls. Upstream history receives an assistant text receipt naming shared photo references, not pixels or browser URLs. `POST /api/gateway/album/resolve` batch-resolves session + role + event IDs and fingerprints without altering history. Completion snapshots and reply recovery enrich by exact reply identity, including photo-only completions; omission of client metadata never erases retained references. If media lookup fails, text history and completion snapshots remain available without the enrichment. The resolver instead returns 503 for retry, never a successful empty result that PWA could mistake for deletion.
- `shenyu_album_open` has a private, in-process view effect. New tool transcript rows and mixed-tool pending rows contain only the photo pointer. Each tool loop owns a request-local memo keyed by call ID and photo ID: resolve/encode each observation once, freeze both success and failure, and return a fresh transport copy on later rounds. A new explicit open can retry a failed read without rewriting the old result. Restored mixed-tool views are resolved once during preparation, after the pixel-free request snapshot has been written; completion snapshots use that earlier snapshot, not the hydrated prepared messages. Pixels and the memo never enter persisted history or request metadata. Keeping an observation stable does **not** mean transmitting it only once: subsequent provider requests still replay the conversation, including that image, for consistent context.
- Anthropic puts observations inside `tool_result.content`; OpenAI-compatible input adds a synthetic user image message after the contiguous tool-result batch. The OpenAI adapter tracks these synthetic positions locally so they neither count as real user turns nor take a user-tail cache breakpoint; no extra marker is sent upstream. Ordinary chat and Anthropic breakpoint policy are unchanged. Sharing does not enter the vision path. Both adapters remove private transport keys, and recursive request-log redaction removes images even inside nested tool results.
- New album writes normalize and validate MIME against the same JPEG/PNG/WebP/GIF allowlist used when opening and serving photos, before creating album rows. This checks declared format, not image decoding. Unsupported legacy rows are not rewritten or deleted and remain rejected at the read boundary. Concurrent creation of the same book name reuses its unique row rather than replacing it; metadata INSERTs name their columns.
...
Tests: `tests/test_album_media.py`, `tests/test_album_delivery.py`, `tests/test_album_review.py`, `tests/test_album_views.py`, `tests/test_album_handoff.py`, `pwa/tests/albumMedia.spec.ts`, `pwa/tests/photoLoader.spec.ts`, `pwa/tests/albumPhotos.spec.ts`, and isolated browser scenarios in `admin/e2e/album-photos.spec.ts`. The Python and PWA slot tests consume the same `tests/fixtures/album_media_slots.json` wire contract, covering both orders of live/expired pairs and every live/expired/cleared permutation. Provider tests capture actual adapter payloads with fake transports; they do not establish a live supplier's acceptance or cache hit rate.
...
offers an explicit `接入线程` action that loads a selected existing session before sending. The PWA's local-first reopening, snapshot fallback for a device with no local record, explicit clean cold-start recovery, reply versions and persisted display history are owned by this document's § Transcript identity and recovery. The configured client context window still limits what is sent to the model; it does not limit how much display history remains on the device. A new or different tag intentionally starts a separate session, so a client
...
to the following upstream tool-result message. The response-detail opt-in does not authorize persistent server request logs. Device display records and bounded runtime tool receipts have separate owners; see § Transcript identity and recovery for their retention and recovery boundaries. Non-streaming responses expose the same list
...
The deployment-only `CHAT_ARCHIVE_BACKEND` selects `supabase` or `sqlite` for original-text capture and `/api/archive/*` reads. Production was migrated to `sqlite` on 2026-09-18 after a paused source export, preview/apply/verify, and backup-restore check; the Supabase archive remains intact as rollback copy. In SQLite mode no chat-archive query/write goes to Supabase, including chat-original Recall and ID reads; Stars, Mem, ordinary indexed Recall, calendar and origin/shared books keep their existing sources. `shenyu_gateway/local_chat_archive.py` owns a separate file, defaulting to `shenyu_chat_archive.db` beside `GATEWAY_DB_PATH`; `CHAT_ARCHIVE_DB_PATH` can explicitly override it, but must not point at the runtime DB. Selection requires an already initialized/imported archive and is checked at startup. There is deliberately no live Admin toggle: switching sources requires migration and verification. `chat_archive_backend` and `chat_archive_db_path` are absent from `ConfigUpdate`; HTTP tests assert that POST cannot mutate or persist them. In SQLite mode, changing `gateway_db_path` through POST is also rejected when it would move the default sibling archive or point the runtime store at the archive. An in-flight writer rejects destination mutation in either direction.

**Agent short path**

`pwa/src/api/client.ts` carries per-version archive envelopes alongside the existing `messages` history → `shenyu_gateway/prepare_messages.py` retains envelopes in the archive/snapshot copies and removes them before model context assembly → `shenyu_gateway/chat_archive.py` projects visible text and captures into the selected Supabase table or `shenyu_gateway/local_chat_archive.py`. `shenyu_gateway/archive_routes.py` serves PWA 回看/档案/搜索 from the same backend. `upstream_client.py` also removes archive-only fields for callers bypassing preparation. Runtime pruning belongs to `store/_admin.py`, not this separate archive. The archive is never used to assemble model history.

**Chat-original Recall shares the selected archive**

`shenyu_gateway/gateway_tools/_recall.py` routes `mode=verbatim` and explicit `chat`/`conversation`/`archive` sources to the same `CHAT_ARCHIVE_BACKEND` as `/api/archive/*`. In SQLite mode, search uses `shenyu_gateway/local_chat_archive.py::LocalChatArchive.recall_candidates` and `shenyu_recall_read` uses `shenyu_gateway/local_chat_archive.py::LocalChatArchive.read_message`; both apply the reader's visible-row rule, excluding tombstones and folded legacy copies. Ordinary indexed Recall, Stars, Mem and heartbeat sources are unchanged. The storage label `source_table` retains its compatibility spelling; it does not select the backend.

Chat Recall still takes up to four literal query terms, fetches at most 200 newest matching originals, and applies the existing tool-layer overlap ranking and result limit. Literal matching uses the same Unicode case-insensitive predicate as the archive reader, without interpreting `%`, `_`, or `*` as wildcards. The reader's single-phrase search and Recall's multi-term scoring remain different interfaces over the same source; neither becomes vector search. Full content and stable `source_id` are returned for read-through. SQLite construction/reads run off the event loop. Missing/invalid local storage returns an explicit unavailable error, never an empty successful result, blank replacement database, or fallback to the stale Supabase rollback copy. Explicit Supabase mode remains supported.
...
`chat_archive.py::archive_visible_text` is the single pure capture projection: strip recognized client extras; for generated replies, remove leading echo/private heartbeat blocks to a fixed point; then trim boundary whitespace. Interior Markdown/whitespace and user-quoted tags stay intact. It neither writes heartbeats nor changes the input transcript. A new stripper must preserve event envelopes and have idempotence, replay, user-quote, partial-recovery, and old-representation compatibility tests. Changing the projection must never mint new IDs or silently rewrite archived originals. Heartbeat extraction/storage and its ID-based backup remain independent.

ID-less legacy capture still uses the global bounded `chat_archive_seen` cache. It dual-reads the old pipeline output and known echo boundary separators (including two leading newlines), and records one canonical digest after successful capture/recognition rather than multiplying cache entries. This prevents the verified echo whitespace replay defect while the old cache evidence remains; it is **not** durable event identity for arbitrary old clients, a cure for same-word legacy occurrences, or automatic repair of unknown historical timestamps. User time markers (PWA status suffix / Operit) and inherited reply clocks remain the legacy fallback; unanchored old text can still use capture time. Do not present that fallback as an exact recovered send time. Never mass-migrate old identities from content similarity.
...
- Both backends keep the existing archive response fields and opaque `event_at|archived_at|id` cursors. Local SQL applies cursor filtering before LIMIT; literal search preserves one/two-character Chinese, Unicode, `%`, `_`, quotes and other literal punctuation. Search has the same 200-result cap as the cloud endpoint (including `limit=0` becoming 1); ordinary message pages keep their separate 1000-row cap. Local day counts are exact and not subject to the cloud reader's 10,000 fetched-row ceiling: a larger local count after cutover need not mean duplication. Compare raw IDs/fields, not truncated calendar counts. It is not a vector search; no claim of production latency improvement has been measured.
- Local readers use an indexed `NOT EXISTS` selection rather than ranking the whole archive with `ROW_NUMBER`. The earlier-copy lookup stays global even when the page has a cursor. Readers run the shared selection directly, so existing v1 files with the old persisted view need no write-on-read schema change. Synthetic query-plan/timing reproduction: `python -m tests.archive_read_benchmark --rows 20861 100000`. Unrestricted text search can still scan history, especially for missing/pure-cased queries; this is not an FTS index or a constant-time-search guarantee.
- Search prefilters only a necessary uncased literal fragment with SQLite `instr`, then uses escaped Python regex + `IGNORECASE` as authority. Blind `LIKE` is not equivalent: SQLite folds ASCII only by default while Python recognizes Unicode case variants. Pure cased queries bypass the prefilter; punctuation, short Chinese and embedded NUL are covered by regression tests.

**Migration and backup (explicit operator actions, not startup side effects)**

The deployment image ships `scripts/local_chat_archive.py`. Python **>=3.12** is already required by `pyproject.toml`; the archive requires linked SQLite **>=3.30.0** (FILTER support). Docker build checks and prints the linked library version, and archive construction rejects an older library before creating any files. This is a capability floor, not a pinned distro or a claim about the currently running VPS version. First verify the actual VPS volume; `/data` below is an example matching the previously observed mount, not a newly verified fact. Do not upload raw runtime DB backups casually: they contain secret overrides. The separate archive file contains no configuration secrets, but its original text is still private.
...
Imports preserve original IDs, text, hashes, timestamps and soft-delete markers. Conflicting immutable fields abort the transaction; re-import never revives a local deletion. Exports/backups refuse to overwrite an existing path and are published from completed private temporary files. Before cutover, test restore by opening that backup and running `verify` against the same frozen source. **`verify` is exact cutover/snapshot evidence, not a long-term health check:** new local messages or new tombstones must fail comparison to an old export. After cutover use database integrity checks and an isolated restore; compare to an export of the same backup snapshot, not the old migration source. `stats` reports counts only and does not certify integrity. Plain backups are **not encrypted** and these commands do **not schedule computer backups**; computer-side pull, encryption, retention and verification must be configured separately. A computer that is off has not received the newest backup. Completed backups are converted to rollback-journal mode (`DELETE`) before publication so the standalone file needs no WAL sidecars on read-only media; the live archive stays in WAL mode. On SQLite >=3.22 a live `mode=ro` connection can recreate `-wal`/`-shm` if the containing directory is writable. Cold-start tests use a new process with no pre-existing sidecars, but the actual VPS volume permissions still require deployment verification. Do not use `immutable=1` on a live-changing archive to bypass that requirement.

IDs must not contain `|` or NUL because the archive cursor uses `|` separators. Supabase UUID IDs satisfy this; an invalid ID causes a read-only import preview to fail before any rows are written. Manual archive writes explicitly use `isolation_level=None` with `BEGIN IMMEDIATE` and commit/rollback; ordinary source SELECT/PRAGMA statements do not implicitly open a write transaction.
