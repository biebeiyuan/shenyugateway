...
New runtime tool receipts retain `reply_version_id` and the real `tool_call_id`, including results reused by the existing argument cache without re-execution. Hydration matches known reply identities and merges missing call phases without moving locally observed offsets or replacing completed results. Unknown `ok:null` may become a known boolean; `false`, zero offsets, empty result strings and null inputs remain meaningful observed values. Legacy null outputs and empty identity labels alone may be repaired. An unchanged merge returns its original array, so repeat hydration is a no-op without property-order-sensitive serialization. Equal reply text does not suppress missing-tool recovery. Only unique identity-less legacy text can use fallback association; ambiguous groups remain unassigned. A failed legacy localStorage write preserves the prior committed value and reports failure; it never strips tool outputs/events to claim success.
...
A write conflict has an explicit **保留本页并重新同步** action that does not call the failing ordinary save first. `useTranscript.ts` invalidates pre-resync queued snapshots and blocks editing/navigation during the operation. It commits a full local recovery copy before reading the latest manifest, retains that saved original too, then CAS-commits a conservative merge before applying and unlocking. A read alone never unlocks a stale writer. Failed checkpoints, changed live input, changed session, missing/corrupt records, or a second concurrent write preserve the live page and fail closed; no blind revision retry is allowed. Only exact known replies with identical body/echo can contribute missing process data; divergent drafts and reply branches stay in their own copies rather than being spliced into the next model request.

Recovery copies are immutable `add` records under typed array keys in the existing `lists` store, separate from replaceable session-list string keys. This additive format needs no database-version upgrade that could block another open tab. They are scoped to gateway/session, survive reopening, and are never pruned as list caches. Settings lists both originals, previews their drafts and exports full messages/versions/process data. **找回这份草稿** checkpoints the current input first, then CAS-commits the selected draft and attachment metadata with `editId=null`; it never sends a message or replaces the active conversation. Ordinary photo-byte retention is unchanged. The visible main record follows the latest saved branch; keeping both originals is not a claim that divergent branches can be automatically combined.
