...
The `slow` (calendar) and `heartbeat` layers share the `system.end` breakpoint, so a fresh heartbeat batch or a new calendar page would otherwise rewrite the whole system prefix every time. `system_prefix_buffer.py::resolve_system_prefix` holds those two layers behind a buffer gate (`prepare_messages.py`, between rendering the layers and assembling them): it keeps the previously served `slow`/`heartbeat` text — stored as `system_prefix_state` alongside `island_state` in `context_window_states` — until either the buffer window elapses (`buffer_seconds` parsed from `ANTHROPIC_CACHE_TTL`, so 1h by default) or the epoch resets on a trim/branch (a rebuild is due anyway). The rule is **added content waits, removed content refreshes now**: a newly written heartbeat or calendar page is buffered so the cached system prefix stays byte-identical and keeps hitting, but deleting an already-injected heartbeat or clearing a calendar page forces an immediate rewrite (line-set difference, the same shape as the island's `inactive_item` force). If the TTL cannot be parsed, `buffer_seconds` is 0 and the gate fails open to today's behaviour (refresh every turn) rather than pinning stale text. This is a passive-injection concern only: 沈予 reading her own heartbeats via `shenyu_read_heartbeat` hits the live table and is never buffered. The decision (`held`/`refreshed` with a reason) rides the content-free `context_window_events` log; Admin's session detail exposes `system_prefix_refreshed_at` so an injected-but-still-buffered heartbeat shows as 缓冲中.
