...
其中 § 交付状态梯与探针边界一节是全仓（不限前端）交付验收的唯一正本。
...
4. 桌面浏览器永远复现不出真机的部分毛病（视口、键盘、滚动惯性）——
   桌面绿灯不作数，手机视口才是第一现场（见本文档 § 交付状态梯与探针边界）。
...
这一小节是整个仓库交付验收的唯一正本：状态怎么填、什么时候探生产、手机视口的地位都在这里。
`AGENTS.md` 的交付日志段和 PWA 铁律段只链接到这里，不再各自复述。
...
探针边界：版本接口、生产日志、生产 `/chat/` 页这类探针只属于 `deployed` / `device_verified`
