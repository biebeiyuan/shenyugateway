...
角色语义色与「昼夜同树、换色不换布局」的手法学自 kimi-manor
...
摆位数学在 `pwa/src/viewer/photoStack.ts`（许可头随文件保留），组件只接事件。
移植时用上游原码在同一帧核对过数值一致；上游 README 的「任意时刻不超过三张可见」
是观感描述而非字面不变量——行程中段退场与进场同时渐变，实体只有两张。
...
4. 桌面浏览器永远复现不出真机的部分毛病（视口、键盘、滚动惯性）——
   桌面绿灯不作数，手机视口才是第一现场（见本文档 § 前端验收铁律）。
5. 盲改配额两针：两次没修中就停手上探针（量测/采样），不许第三针盲赌。
6. **修饰词类名不许自带盒模型**（2026-08-30 档案日历事故）：`.active`、`.today`
   这类挂在别人身上的类只管颜色与状态，不设 `padding` / `height` / `margin`；反过来，
   一个已经有独立规则、并自带盒模型的类名（如空状态块 `.empty`）不能再被当修饰词
   复用。scoped style 让人以为类名只在组件内有效——那是对的，但**同一个组件内部照样
   会撞**，而且撞的两条规则各自都对，只读源码看不出来。症状是某个盒子被撑成几倍高；
   查法是看 `getComputedStyle` 的计算值，不是看 CSS 源码。全站扫过一次，当时只有
   这一处，所以旧代码无需整理，新代码起名时避开即可。
...
交付状态怎么填、什么时候探生产，正本在 `docs/DELIVERY.md`；这里只放前端专属的部分。

PWA 三铁律——(1) `npm test` / `npm run build` / HTML 断言只证明技术层，不代表设备结果；
(2) 只有带可识别当前构建的生产 `/chat/` 页是验收对象，本地端口只是预览；(3) 没在受影响设备
复现原场景就报 unverified，不从容器健康、缓存更新或部分测试推断成功——只适用于「PWA
用户可见修复交付生产」这一类交付，不套用到纯后端或未到 `deployed` 档的改动。

圆圆默认在手机端：用户可见问题的复现与验收以手机视口（约 390px 宽）为第一现场，桌面视口
只是补充。「是缓存」只是假设——出口前必须先用响应头或产物哈希拿到证据，且同一个症状不允许
用缓存解释第二次。
...
语义色，但不另开常驻面板。Thinking 与回响的详情正文**不套框**——没有边框、没有底色，用正文衬线字
和留白分隔（2026-08-30 圆圆要的「Claude 原生的感觉」）。粉色留在回响弹层本身作身份色，正文里
再套一层白框就是「框中框」。工具的调用参数与结果仍然留框，那是真代码，框住更好读。点开过程条后，回响在自己的详情面板中显示，长文本必须换行且不能造成
横向溢出。回响是对应 assistant 消息的一部分：本地 transcript、assistant roll variant 和会话交接
都保留它；它不是“最近一条回响”列表，也不因为页面刷新而消失。验收时用 390px 手机视口确认过程条、
粉色详情面板、空正文但有回响的回复，以及刷新/交接后的保留状态。
