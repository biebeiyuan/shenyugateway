...
| ⑤ | PhotoStack 堆叠卡 | 进行中（2026-08-30 起，与看图器同批） |
| ⑥ | 展开 / 收起动画 | 未开工 |

**当前进度（2026-08-30）**:①②③④的持久化半、过程条交错渲染都已上线
（`master` 至 `15ea307`）。正在做：看图器 + 堆叠卡。③ 已降级不做。
下一步顺序：看图器 → 堆叠卡 → 展开收起（⑥ 开工前单独讲边界，它要动消息列表
布局和滚动容器，会跟 `scrollToBottom` 和按块渲染碰上）。
