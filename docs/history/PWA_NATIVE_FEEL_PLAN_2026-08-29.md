...
| ③ | 原生手感 | **降级，暂不做**（2026-08-30 圆圆：没有特别感觉） |
| ④ | 图片持久化 + 看图器 | 2026-08-30 完成 |
| ⑤ | PhotoStack 堆叠卡 | 2026-08-30 完成 |
| ⑥ | 展开 / 收起动画 | 暂不做（圆圆选 C，见下） |

**当前进度（2026-08-30 收工，`master` 至 `4908307`）**

已上线：① 报错护栏 ② 渲染拆分 ④ 图片持久化 + 看图器 ⑤ 堆叠卡，以及过程条交错
渲染、首屏只画末 20 条、主壳拆出两个 composable、工具文案按完整名精确匹配。

- ③ 原生手感：**降级不做**（圆圆看过清单说没有特别感觉）。页面缩放刻意不禁。
- ⑥ 展开 / 收起动画：**暂不做**。圆圆选了 C（点堆叠卡直接进看图器），理由是那已经
  覆盖「看全部图」，而 ⑥ 最贵、最容易出岔子——上游只有规格没有代码，且要动消息列表
  布局与滚动容器，会跟 `scrollToBottom` 和按块渲染碰上。真要做，展开后的落点形态
  要先定（竖排大图 / 网格），那是产品决定不是实现细节。

**还挂着的一件事**：上游 relay 偶发 `Unknown image format: image/jpeg;base64`。
已验证我们发出的 `media_type` 是正确的 `image/jpeg`，是 relay 内部转换切错了分号。
当时决定先观察频率再定要不要做窄重试——因为 blob URL 那个 bug 会让同一批请求反复
失败，两种错误在日志里混着，数不准。那个 bug 已修（`3b776a9`），所以现在的日志
可以拿来数了。不要为它特化核心逻辑（`AGENTS.md` § Project Memory 那条）。
...
## ③ 原生手感（2026-08-30 降级，暂不做）

圆圆看过这份清单后说"我的需求好像不是特别需要，没有什么特别的感觉"。所以这一档
不再排期，下面的清单留作以后真有体感问题时的线索，不要当成待办。

他真正提的那件事是**页面可以放大缩小**，不在原清单里。结论是**不禁**：
`user-scalable=no` 会连带禁掉双指缩放（无障碍功能），而且 iOS 10+ 直接忽略它，
会造成安卓能锁、iPhone 锁不住的不一致。真正让人觉得"这是网页"的是双击文字意外
放大和点按钮时页面跟着缩——那两个可以单独治，同时保留双指缩放。看图器内部是例外：
它自己要处理双击放大和双指缩放，两套手势会打架，所以只在看图器打开期间锁页面缩放。
