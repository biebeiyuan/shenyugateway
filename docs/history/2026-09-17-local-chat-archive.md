...
## SQLite runtime repair and production cutover (completed 2026-09-18)

PR #3 (`c357f7d80b37608b7202ace5bb6a29af1f070961`) normally merged the repaired image. It builds the official SQLite 3.51.3 shared library from a pinned source archive, verifies the `sqlite3.c` SHA3-256, and asserts the loaded Python source ID and dynamic link path in image build and CI. Production verification shows Python SQLite 3.51.3, source ID `2026-03-13 10:38:09 737ae4a34738ffa0c3ff7f9bb18df914dd1cad163f28fd6b6e114a344fe6d618`, and `_sqlite3` linked to `/opt/sqlite/lib/libsqlite3.so.0`.

This is an environment-only change: it does not alter chat history, context assembly, trimming, cold start, Memory Island, Stars, Mem, tools, or session handoff.

During the approved cutover, one gateway instance was stopped after a 30-second in-flight drain. The runtime database `/data/shenyu_gateway.db` remained separate and unchanged. A private source snapshot at `/data/source-chat-archive.jsonl` contains 20,904 Supabase rows including soft deletes (mode 600; SHA-256 `5b4a5e141cedaddfa7c14d7ef12d54a714c9b1b24494bb609478f9466189f729`). Preview, apply, and frozen-snapshot verification all passed: total 20,904, active 17,654, visible 12,454. The archive database is `/data/shenyu_chat_archive.db`; backup `/data/backups/shenyu_chat_archive-pre-cutover.db` has SHA-256 `1cff3005ed4556445e9826a157ae762b20f7a305079048f67066e268c397e9f4`. A private restore copy passed `PRAGMA integrity_check`, cold reopen, stats, and the same frozen-snapshot verify. The cloud archive remains intact as rollback copy; no dual-write or deletion was performed.

Coolify's persistent production environment now sets `CHAT_ARCHIVE_BACKEND=sqlite` and `CHAT_ARCHIVE_DB_PATH=/data/shenyu_chat_archive.db`, with normal archive capture enabled. Deployment `r8ku466p5x3azgud8x0o5max` finished healthy on the repaired image. Authenticated `/api/archive/days`, `/api/archive/messages`, and literal Chinese `/api/archive/search` returned 200. Supabase-backed Stars, Mem, Recall, calendar and origin/shared books remain enabled. An isolated restored copy also accepted a new ID-based write and passed cold-start integrity; production capture still follows the existing later-request/fire-and-forget path, so per-message identity, immediate terminal-reply capture and reply recovery remain pending and are not claimed here.
