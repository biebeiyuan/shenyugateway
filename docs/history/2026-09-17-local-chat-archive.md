...
## SQLite runtime repair follow-up (in progress)

The production cutover is intentionally blocked until the image's Python `_sqlite3` loads a runtime with an explicit WAL-reset repair. The base image's Debian SQLite 3.46.1 package proved the SQL capability threshold but did not provide source or backport evidence for that fix. The follow-up builds the official SQLite 3.51.3 shared library from a pinned source archive, verifies its `sqlite3.c` SHA3-256, and asserts the loaded Python source ID and dynamic link path in both the image build and CI.

This is an environment-only change: it does not alter chat history, context assembly, trimming, cold start, Memory Island, Stars, Mem, tools, or session handoff. It does not migrate or switch the archive backend. The existing runtime database and Supabase archive remain untouched until the repaired image is deployed and the approved pause/export/preview/import/verify/backup sequence is completed.
