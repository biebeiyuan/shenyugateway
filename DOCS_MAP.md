...
地图以现行源码的职责、数据流和约束为事实；任务入口只是索引，不用操作步骤替代现状，也不把计划写成已实现。这份文件说明从哪里读、哪份是正本，规则不只留在某个模型的全局记忆里。
...
| `docs/DELIVERY.md` | 全仓交付验收的唯一正本：四档状态梯、探针边界、验证基线与施工簿记法、负结果格式 | 交付状态档位、验收证据要求、探针边界或负结果格式改变时 |
...
- 优先更新已有正本。入口只给“任务 → 正本章节/代码”；细节在正本写一次，其他入口缩成指针。新主题确无归属且独立维护时才新增 Markdown，不为一次审查再加地图层。
- 设计稿应标明阶段或历史属性；审查报告是某个时间点的快照，不自动成为长期事实。
- 跨模块字段在所属正本追清“谁写 → 存什么 → 谁读 → 缺失怎么办”，并连到行为测试；不逐字段复制 schema。路径检查只证明锚点存在，语义仍须核对代码。
