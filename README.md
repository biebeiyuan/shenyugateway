...
- `pwa/src/session/reconcile.ts`: tail-only reconciliation after a background disconnect — matches `reply_version_id` when available, falls back to the local last-user anchor for legacy rows, and restores the server-drained reply without replacing the whole transcript.
