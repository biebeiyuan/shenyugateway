...
- `admin/src/views/MemoryGraphView.vue`: personal memory-graph console — paper-style typographic map of anchors and red-thread relations with event-day warmth plus a recent-activity stream, clickable anchor cards listing the originals that mention each anchor with management forms folded away, entity/alias/relation management with candidate confirmation and an archived-anchor drawer, unanchored name candidates, historical exact-alias backfill, and a read-only Recall preview that mirrors what the model actually receives — literary recall groups (脱口而出/由此及彼/浮想) over evidence-highlighted source cards with explicit source-anchor linking (the preview response only adds the query terms used for highlighting, never scoring internals).
