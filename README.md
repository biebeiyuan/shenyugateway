...
- `pwa/src/session/reconcile.ts`: automatic recovery after a background disconnect or session handoff — repairs the current reply of the latest user request and preserves the rest of the transcript. It matches the local bubble's `reply_version_id` against the server's reply and falls back to the single newest reply for legacy rows without one; historical roll variants stay local state, because repeated user text cannot identify which past request a reply belonged to. A strict containment guard is the only write gate, and it compares against the server's own candidate rather than the bubble it is repairing — comparing the repaired bubble to itself always succeeds and would clear `truncated` on a rejected candidate, stopping the retry chain while the text is still half a reply. An unrelated or shorter candidate is dropped entirely: nothing is written, no variant is added, and `truncated`/`error` stay so the backoff chain keeps asking until the gateway finishes draining. A server version that already matches the local text still clears them, because that is a confirmed-complete reply. EchoSegments, thinking, and local tool events are preserved when present locally. The gateway recovery bridge reads request-level replies from `gateway_messages`; PWA calls it on open, foreground return, and stream failure.
