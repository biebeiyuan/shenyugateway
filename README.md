...
- `pwa/src/session/reconcile.ts`: automatic recovery after a background disconnect or session handoff — repairs the current reply of the latest user request and preserves the rest of the transcript. It matches the local bubble's `reply_version_id` against the server's reply and falls back to the single newest reply for legacy rows without one; historical roll variants stay local state, because repeated user text cannot identify which past request a reply belonged to. A strict containment guard ensures recovery can only extend local text; unrelated or shorter candidates remain switchable variants without replacing the current bubble, and a server version that already matches the local text still clears `truncated`/`error` so the retry chain stops. EchoSegments, thinking, and local tool events are preserved when present locally. The gateway recovery bridge reads request-level replies from `gateway_messages`; PWA calls it on open, foreground return, and stream failure.
