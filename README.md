...
- `pwa/src/session/reconcile.ts`: automatic recovery after a background disconnect or session handoff — repairs the current reply of the latest user request and preserves the rest of the transcript. Both recovery paths require the local reply/archive identity to match the candidate; only genuinely unversioned legacy rows may use the latest-user text anchor. A known version missing on the server is not permission to fall back to a different Roll; historical roll variants stay local state, because repeated user text cannot identify which past request a reply belonged to. After identity validation, a strict containment guard is the content write gate, and it compares against the server's own candidate rather than the bubble it is repairing — comparing the repaired bubble to itself always succeeds and would clear `truncated` on a rejected candidate, stopping the retry chain while the text is still half a reply. An unrelated or shorter candidate is dropped entirely: nothing is written, no variant is added, and `truncated`/`error` stay so the backoff chain keeps asking until the gateway finishes draining. A server version that already matches the local text still clears them on the bubble and selected variant, preserving any recovered archive identity, because that is a confirmed-complete reply. EchoSegments, thinking, and local tool events are preserved when present locally. The gateway recovery bridge reads request-level replies from `gateway_messages`; PWA calls it on open, foreground return, and stream failure.
