...
- `shenyu_gateway/doc_touchpoints.py`: learns from `git log` which documents historically changed alongside a source file, and prints them as advice for the files you just touched. Deliberately never fails and never blocks — whether a document must follow a code change is a judgement call, and a red light would teach editing one line of prose to silence it. Its module docstring records the measurement that chose co-edit history over prose name-matching (precision 36% vs 17%) and the two blind spots it names rather than hides: a file too new to have taught anything, and a file with plenty of history but no document steady enough to clear the bar. Both matter because the pointer list never says which changed files it came from, so a report advising on two of four files used to read as though it covered all four. `scripts/project_delivery.py record` prints this report beneath a fresh delivery entry, from the paths that entry declared.
...
- `shenyu_gateway/context_snapshots.py`: reply-completion snapshot writer; identity is persisted before resend for handoff and recovery. See `docs/architecture/REQUEST_CONTEXT.md` § Transcript identity and recovery.
...
- `shenyu_gateway/local_chat_archive.py`: independent SQLite original-text store, legacy read-view folding, literal search, chat Recall candidates/ID reads, stable-ID import/verification and consistent backup; never a model-history provider.
...
- `shenyu_gateway/room_tools.py`: room mode tool definitions, execute dispatch, door count collection, and the Room-to-canonical-windowsill bridge (`origin=room` plus idempotent legacy scribble import). Room mode exposes visible doors as individual schemas and has no broker of its own — the `room_broker_tool` that once built one was never called by any surface and was removed.
- `shenyu_gateway/room_scenes.py`: window scenes (weather, atmosphere, landscape). Change scene copy here only.
- `shenyu_gateway/room_newspaper.py`: fixed RSS sources, feed parsing, issue rolling, optional quality checks, and draft generation. This is the **editorial** side (fetch, judge, publish); the read side is `newspaper_basket.py`, deliberately not here — this module pulls in `upstream_client`, which imports `tool_registry`, which imports `gateway_tools`, so a read primitive living here cannot be imported by the daily-chat mixin without a circular import.
- `shenyu_gateway/newspaper_basket.py`: the basket **read** primitive plus the one tool description both doors hand out. Shared by `room_tools.py`'s `room_newspaper_basket` and `gateway_tools/_newspaper.py`'s `shenyu_newspaper_basket`, and it imports neither — depending on nothing but a duck-typed store is what keeps the dependency arrows one-way. Wording lives in `NEWSPAPER_BASKET_DESCRIPTION` because the two doors drifted apart once; `tests/test_room_newspaper.py::test_both_basket_doors_hand_out_the_same_instructions` fails if either grows its own copy.
- `shenyu_gateway/gateway_tools/_newspaper.py`: `shenyu_newspaper_basket` — the old-newspaper basket read from ordinary chat. Wraps `newspaper_basket.read_newspaper_basket` and deliberately records no `room_trace`, because `last_room_visit_at()` takes that table's newest row regardless of action and would damp the room charge into hiding doors on his next real visit. `MEMORY_ROOM.md` § Old newspapers in daily chat owns the reasoning.
...
Ordinary chat pixels remain a **device-only newest-30 cache**; only explicit album saves keep pixels on the server indefinitely. Evicting device bytes or removing images from the model's two-user-turn window must not delete an album photo. PWA can recover a saved original attachment by its byte fingerprint and then use its stable photo reference; unsaved evicted images display `本机图片已清理`, while network/storage failures offer retry rather than claiming deletion. This does not manufacture missing old bytes or recover a legacy message whose attachment identity was never retained.

`shenyu_album_list(book?, limit?, cursor?)` defaults to actual photos across books. Each row contains only `title` (existing book name, not a new per-photo title), `content` (unchanged note and mood separated by a newline), and `photo_id`; page-level `next_cursor` is null at the end. A cursor is bound to the book filter and stable `(saved_at, id)` descending order, so concurrent newer saves do not repeat earlier pages. `shenyu_album_open(photo_id)` explicitly presents pixels to the model without sharing; `shenyu_album_send(photo_id)` explicitly shares a reference to PWA without opening pixels for the model. List → send is valid; neither list nor open automatically shares. These actions are also exposed by the existing broker in both full and daily surfaces; short aliases route to the same handlers. The original save/note-sync behavior and existing read-only Admin directory API remain compatible.

Shares belong to an exact session + reply identity + tool-call identity. They are committed before the client event, survive refresh/session recovery, and stay attached to their own re-roll version. Replay of the same share is idempotent; a new reply may share the same photo again. Only metadata is retained for ordinary attachments: user-side restoration can resolve a saved fingerprint without uploading pixels again, whereas share history is server-authoritative. Transport, trust boundaries and observation lifetime are owned by `docs/architecture/REQUEST_CONTEXT.md` § Album media transport.


- `shenyu_gateway/store/_album.py`: local SQLite `album_books` / `album_photos` — image bytes as BLOBs on the mounted volume, keyset paging and fingerprint lookup; `album_message_media` retains only immutable user attachment metadata and per-reply share references, independently of runtime snapshot pruning. Listing queries never select the blob column, so browsing the album does not move megabytes. Image bytes never reach Supabase or the request log.
- `shenyu_gateway/gateway_tools/_album.py`: `shenyu_album_save` / `shenyu_album_list` / `shenyu_album_open` / `shenyu_album_send`, image-block decoding for both the OpenAI data-URL and Anthropic `source.data` shapes, and the note sync to Supabase `shenyu_album_notes` plus Recall indexing. `latest_turn_images()` reads the newest image turn from the messages instead of requiring a marker in the body — writing one would change history normalization and make branch detection reset the prompt-cache epoch. A failed note sync leaves the photo saved; only its searchability is lost.
- `shenyu_gateway/album_media.py`: metadata validation and reference rendering, server-authoritative share effects, exact-event recovery, upstream-only text receipts and transient vision hydration; never stores ordinary chat pixels.
...
- `shenyu_gateway/gateway_admin_routes.py` `GET /api/gateway/album` and `GET /api/gateway/album/photo/{id}`: read-only legacy directory and one authenticated immutable bytes route restricted to JPEG/PNG/WebP/GIF; `POST /api/gateway/album/resolve` is a read-only metadata lookup for PWA fallback and recovery.
...
- `admin/src/views/ConfigView.vue`: configuration page, including the 回响 (Echo) enable switch, saved prompt, and retention controls. Switch behavior is defined in `docs/architecture/REQUEST_CONTEXT.md` § Context Layers.
...
- `scripts/doc_touchpoints.py`: run it before handing off to see which documents historically changed with the files you touched (`--staged` for staged changes only, bare paths to ask about specific files). Advice only, always exits 0. `project_delivery.py record` runs it for you on the paths a delivery declares, so this stays for asking mid-work rather than being the only door.
...
- `pwa/src/session/transcriptStore.ts`: per-gateway/session IndexedDB manifests, changed-message transactions, stale-writer checks, one-time legacy migration and retained branch rows.
- `pwa/src/session/useTranscript.ts`: serialized save checkpoints, local restore, draft/reading-position state, storage failure reporting and owner export.
...
- `pwa/src/session/variants.ts` / `pwa/src/session/persistence.ts`: reply-version state machine, metadata-only transcript codecs and legacy localStorage compatibility. Current storage and recovery contracts live in `docs/architecture/REQUEST_CONTEXT.md` § Transcript identity and recovery.
- `pwa/src/session/media.ts`: validated attachment codecs, exact-reply share event binding and display/wire separation.
- `pwa/src/session/photoLoader.ts`: local-first restore, read-only server reference resolution and authenticated album image loading; owns transient display URLs, retries, concurrency bounds and stale-session/variant guards.
- `pwa/src/session/photoStore.ts`: IndexedDB store for chat photo bytes, keeping the newest `STORED_PHOTO_LIMIT` (30) and reporting which ids it evicted. `photoDataUrl()` rebuilds a data URL for restoring a bubble — deliberately not `URL.createObjectURL`, whose `blob:` address is valid only inside that browser process and reached upstream as an unfetchable link once written into `attachment.dataUrl` (2026-08-30 production 500: `illegal base64 data at input byte 0`). Bytes are stored as `ArrayBuffer` rather than `Blob` (structured clone is uniform for it, and Blob-in-IndexedDB has a history of engine bugs) and re-wrapped as a Blob on read. `photoFingerprint` is SHA-256 over the bytes, the same algorithm as `shenyu_gateway/store/_album.py::photo_fingerprint`; both sides assert the same known digest so a change to either goes red. This bounds ordinary device bytes; saved photos fall back to the album through `photoLoader.ts`. Model input independently follows the gateway's newest-two-turns rule; remote display URLs are never assigned to `attachment.dataUrl`.
...
- `pwa/public/manifest.webmanifest` / `pwa/public/sw.js` / `pwa/src/main.ts`: installable PWA shell; the service worker never caches `/v1/`, `/api/`, or the deployment-proof `/chat/build-info.json` response, and registers a stable worker URL without forced takeover/reload. Paired build precaching and restore ownership are defined in `docs/architecture/REQUEST_CONTEXT.md` § Transcript identity and recovery. `main.ts` also installs the app-level `errorHandler`, the last stop for anything the per-row guard does not cover.
...
- `pwa/vite.config.ts` / `pwa/scripts/`: isolated development server on port `5174`, build identity and paired shell/asset precache manifest emission, with build assertions for the bundle and worker identities.
...
- Chat photos live on the device that sent them: bytes in IndexedDB (newest 30), metadata in the per-session transcript store, so a refresh keeps the transcript and the recent images while older bubbles show 图过期了. This is separate from the gateway's own newest-two-turns image trim and from the album, which never expires. A message accepts up to 9 photos.
- Cross-client handoff uses the existing `X-Shenyu-Session-Tag`: `X-Shenyu-Client` identifies the surface, while the session tag keeps Operit and PWA on one gateway thread. PWA restores the target session's own device record first and supplements it by identity; a session not held locally uses `context_snapshots`, with inspection rows only as the legacy fallback. `/chat/?session_tag=<tag>` remains an exact handoff entry. Device display retention is independent of the unchanged gateway context window and cold-start bridge retirement. Detailed merge, migration and list-visibility contracts live in `docs/architecture/REQUEST_CONTEXT.md` § Transcript identity and recovery.
...
| Room | 房间 / 窗台 | `private_capture.py`、`room_context.py`、`room_tools.py`、`room_newspaper.py`、`newspaper_basket.py` | `pwa/src/App.vue`、`pwa/src/meta/roomEntry.ts`、`admin/src/api/room.ts`、`RoomView.vue`、`views/room/` | `MEMORY_ROOM.md` § Room Mode |
| 相册 | 沈予的相册、收藏的图 | `shenyu_gateway/store/_album.py`、`gateway_tools/_album.py` | `shenyu_album_list/open/send`、`GET /api/gateway/album/photo/{id}`、`POST /api/gateway/album/resolve` | 本文件 § Album |
...
An installed PWA does not switch versions the moment you deploy, and an update must not force a reload of an active chat. `docs/architecture/REQUEST_CONTEXT.md` § Transcript identity and recovery owns paired-build precaching, waiting-worker activation, per-session persistence and legacy-data migration. Verify the active build in Settings after a normal close/reopen; do not clear browser site data as an update procedure.
