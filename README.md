...
An installed PWA does not switch versions the moment you deploy, and an update must not force a reload of an active chat. `docs/architecture/REQUEST_CONTEXT.md` § Transcript identity and recovery owns paired-build precaching, waiting-worker activation, per-session persistence and legacy-data migration. Settings distinguishes the running page, its controlling offline build and a downloaded waiting build; do not clear browser site data as an update procedure.
