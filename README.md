...
- `pwa/src/session/reconcile.ts`: automatic recovery after a background disconnect or session handoff — merges request-level durable assistant replies in the latest same-user roll group into one switchable bubble, matches `reply_version_id` when available, falls back to the local last-user anchor for legacy rows, and preserves the rest of the transcript. Only-increase guard ensures server content never shortens local text (包含关系 + 长度下限); echoSegments and thinking are preserved when present locally. The gateway recovery bridge reads request-level replies from `gateway_messages`; PWA calls it on open, foreground return, and stream failure.
