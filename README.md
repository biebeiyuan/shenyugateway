...
- `shenyu_gateway/memory_graph.py`: temporal personal memory graph service for confirmed entities, aliases (create/confirm/delete), source mentions, typed relationships, exact-alias backfill, one-hop Recall candidates, and unanchored name-candidate aggregation from mem-note fields.
...
- `admin/src/views/MemoryGraphView.vue`: personal memory-graph console — SVG constellation view of anchors and relations, entity/alias/relation management with candidate confirmation and an archived-anchor drawer, unanchored name candidates, historical exact-alias backfill, and the ripple-style read-only Recall preview with explicit source-anchor linking.
...
- `pwa/src/api/client.ts` / `pwa/src/api/presets.ts`: gateway HTTP layer — PWA identity headers, models/sessions/config/chat-stream fetchers, outbound message wiring — plus reading the Console-shared `shenyu_upstream_presets` storage. The Max effort tier sends explicit Anthropic Thinking (`type=enabled`, 32768-token budget); lower tiers retain the adaptive gateway path.
