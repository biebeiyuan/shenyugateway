...
- `pwa/src/App.vue`: the mobile chat surface (ChatNest-derived; `docs/frontend/STYLE_AND_CRAFT.md` § 风格血统声明 records what was borrowed and the private asset pack's boundary) — session/model/preset state, streaming orchestration, per-trace detail view, Projects/Artifacts/Memory/Diary workspace shells, gateway-backed Recents, edit/retry actions, local assistant roll variants, clean cold-start recovery, an image/Room `+` menu, and the shared reply-meta line for Room time plus context/cache/first-round-hit/heartbeat status. Echo content is retained in the transcript and rendered through the existing process view, not a standalone panel. The Vue shell owns UI state and orchestration only; protocol, history, and persistence logic live in the modules below, and one transcript row's own rendering lives in `pwa/src/components/`.
...
Pass the source SHA as the Docker build argument `SOURCE_COMMIT` (or set the runtime environment variable with the same name); the image intentionally omits `.git`. **This is not cosmetic.** Without it the resident home snapshot cannot show the deployed revision, and `/chat/build-info.json` reports `revision: "unknown"` — leaving the build timestamp as the only way to tell which code a device is running. Observed on 2026-08-30: production was serving `revision: "unknown"`, which turned a PWA bug report into guesswork about which build had sent the affected photo. Verify with `curl -H "Authorization: Bearer <token>" https://<host>/chat/build-info.json` after a deploy; if `revision` is not a commit SHA, the build arg is not reaching the builder.
