...
const newspaperGenerating = ref(false)
const newspaperPublishing = ref(false)
...
const noteDraft = ref('')
const newspaperIssues = ref<NewspaperIssue[]>([])
...
const draftNewspaper = computed(() => newspaperIssues.value.find((issue) => issue.status === 'draft') || null)
...
async function loadNewspapers() {
  ...
  try {
    ...
    newspaperIssues.value = data.issues
  } catch {
    newspaperIssues.value = []
  } finally {
...
async function makeNewspaper() {
  newspaperGenerating.value = true
  try {
    ...
    newspaperGenerating.value = false
  }
}
...
async function putNewspaperOnSill() {
  ...
  if (!issue) return
  newspaperPublishing.value = true
  try {
    await publishNewspaper(issue.id)
    message.success('报纸已经放到窗台上了')
    await Promise.all([loadNewspapers(), loadPreview()])
  } catch {
    message.error('放到窗台失败')
  } finally {
    newspaperPublishing.value = false
  }
}
      ...
      <section class="newspaper-band">
        <div class="section-head newspaper-head">
          ...
          <div class="newspaper-actions">
            <NButton size="small" :loading="newspaperGenerating" @click="makeNewspaper">
        ...
        <NSpin :show="newspaperLoading || newspaperGenerating || newspaperPublishing">
          <template v-if="displayedNewspaper">
            ...
            <details v-if="draftNewspaper" class="newspaper-details">
              ...
              <div v-if="newspaperFailures.length" class="source-failures">
                <div v-for="source in newspaperFailures" :key="source.source_id">
