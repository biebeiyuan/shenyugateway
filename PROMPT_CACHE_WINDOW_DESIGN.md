...
11. 闲置超过本轮上游缓存 TTL、缓存反正已凉时，滚一次 epoch（`reset_reason = "cold_cache_rebuild"`）把上下文窗口提前裁到位：`message_high_water` 那一刀迟早会落，与其让它砍在未来某个缓存还热的时刻白白作废一个热缓存，不如趁这次反正已凉一次裁到位。阈值按协议取（Anthropic 用 `anthropic_cache_ttl`、OpenAI 用 `openai_cache_ttl`）。这只滚窗口 epoch、重算岛锚点位置，**不**强制重写记忆岛内容（见 § 5.4）——闲置久是缓存信号、不是内容信号。由 `epoch_reset_on_cold_cache` 控制，默认开；关掉即退回只按水位裁。首轮无上一轮 `last_active_at`，不算冷。
...
- 用户直接写出 active `star_id`，或复述只属于一颗星的独特原句；这两种硬点名无冷却。
- “回忆意图 + 唯一 Star 候选”使用每颗星默认 8 个真实用户轮次的软冷却，由 `STAR_SOFT_DIRECT_COOLDOWN_TURNS` 调整，设为 0 表示不冷却。
- 当前 Star lane 条目被编辑、归档或删除。
- Mem 的 entity 命中、承诺或明确状态变化沿用各自的直接候选规则。
- 空集合与非空集合发生切换。

`branch` 与 `message_high_water` 绕过两条 lane 的粘性：前者表示更早对话被语义修改，后者表示消息窗口越过高水位并发生裁剪。任何一次重写都直接采用本轮完整评分提案和顺序，不在重写后再保留旧岛的 `2/3`。`cold_cache_rebuild` 刻意**不**在此列：闲置过久只说明缓存凉了、不代表旧岛内容失效，那一刻缓存本就全过期、锁岛也无缓存可保，故岛照常走粘性；epoch 重置只按新窗口重算 `anchor_offset`（位置），lane 内容保留。真需要换岛时，正常信号（overlap 跌破阈值、内容变化、条目失活、硬点名）会在同一轮触发，与这次 epoch 重置自然一起换。
...
- 普通星星关键词命中不绕过 2/3 滞回阈值；只有 hard direct、已过 `STAR_SOFT_DIRECT_COOLDOWN_TURNS` 冷却的 soft direct、旧岛星失效、branch 或 message high-water 才立即换岛。
