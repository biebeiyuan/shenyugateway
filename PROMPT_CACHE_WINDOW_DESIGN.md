...
11. 闲置超过缓存 TTL、`system.end` 前缀本轮无论如何要冷重建时，搭这次冷重建的便车顺手滚一次 epoch（`reset_reason = "cold_cache_rebuild"`）：`message_high_water` 那一刀迟早会落，与其让它砍在未来某个缓存还热的时刻白白作废一个热缓存，不如趁这次反正已凉一次裁到位。由 `epoch_reset_on_cold_cache` 控制，默认开；关掉即退回只按水位裁。首轮无上一轮 `last_active_at`，不算冷。
...
`branch`、`message_high_water` 与 `cold_cache_rebuild` 绕过两条 lane 的粘性：前者表示更早对话被语义修改，第二个表示消息窗口越过高水位并发生裁剪，第三个表示闲置过久搭冷重建便车滚了 epoch。epoch 一换岛锚点本就要重定位，趁这次一并刷新到位——force 只放行不强改内容，岛渲染文本没变则版本号不动、缓存照命中。任何一次重写都直接采用本轮完整评分提案和顺序，不在重写后再保留旧岛的 `2/3`。
