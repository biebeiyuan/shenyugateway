...
- 小岛显示的是**实际渲染并发送给模型**的星星和 Mem，不是候选列表。开启完整请求日志时，Stars 和 Mem 会用同一套卡片分别展示当前内容、新增、更新和移除；更新显示为“原来 → 更新后”。旧日志仍显示完整小岛原文。
...
- 页面保留供应商原样返回的 `cached/input/output/write` 数值；当本轮输入分母也可靠时，会追加缓存读取占输入比例。这个百分比不是跨请求命中率，也不是账单节省比例。
