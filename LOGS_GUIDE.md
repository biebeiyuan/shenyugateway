...
- 「今天的小突起」是岛后面那条尾巴：沈予今天已经记下的东西（星星、便签、日历）各留一行回执，用来避免同一件事记第二次。它不进岛正文，所以岛文本的重合门不受它影响；这一条渲染成什么样由 `inject_island_bumps` 和 `island_bump_limit` 两个配置控制（Admin 在 Mem 页的「小突起」组）。
