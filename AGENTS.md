...
- 盼圃 must never gain a reminder. Nothing about it may write into a context layer, and no fruit may expire, be auto-archived, or be marked overdue — a fruit with no `due_on` hangs indefinitely, and one past its date just keeps hanging. This is the only thing separating it from a mem note's `remind_on`, which does force a Memory Island rewrite on its day, so the failure mode here is helpfulness: "surface the fruit that's due today" reads like the obvious missing feature and would quietly turn the wall into a second reminder system. The resident's own words for the distinction are 闹钟催人，墙等人. `README.md` § 盼圃 (Orchard) owns the mechanism, including why the picked condition is frozen on the row and why only picked fruits enter Recall.
...
- **`master` is production.** Coolify auto-deploys that branch, so a `master` push ships to the gateway 沈予 and 圆圆 are actually living in — with the real Supabase and the real mounted data volume. Ask the owner before pushing it unless they have already said to. Feature branches are not deployed and carry no such weight. `docs/DELIVERY.md` § 交付状态梯 owns the rest: the four status tiers, the evidence each one needs, and why a push receipt is not yet `deployed`.
...
The same six locations apply in reverse when you delete the code a field gated: walk the list back out, or the field stays behind as an empty slot — parsed, validated, persisted, and read by nothing. `tests/test_config_update.py::test_every_runtime_config_field_is_read_by_something` is the backstop; it fails on a `RuntimeConfig` field that no code outside those plumbing files ever reads, and it does not count a value merely echoed into a report dict such as `/health` as a reader. On 2026-06-23 `dd30268` removed inline `[mem]`/`[star]` capture and left four fields behind, two of them still rendering switches in `StarsSettingsPanel.vue` — the resident could flip 自动捕获 and be told 保存后生效. Never leave a control in Admin that changes nothing.
...
`params["order"]` has the same shape of gap and the same home: sort through `apply_order` instead of writing the comparison per fake. Measured on 2026-08-30, the hand-written version keyed on `str(row.get(field) or "")`, which cannot tell a NULL from an empty string and so put null-valued rows first where `nullslast` puts them last — 盼圃's dateless fruits, the ones the feature exists for, sorted ahead of the ones actually coming due with every test green. It also compared integers as text, making `version.desc` answer 2 before 10. `tests/test_supabase.py` fails on a fake that sorts on `order` by hand and on any `order` modifier the helper does not model.
