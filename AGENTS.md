...
- When a rule or fact gains a new authoritative home, search the live documents for older copies of it and leave exactly one detailed original. An older location that still has entry value shrinks to a pointer that carries meaning — name the document, the real `## `/`### ` heading, and what the reader will find there, so the pointer is worth following without being followed; a bare "see X" is not. Copies that no longer serve as an entry are deleted. Do not skip the search because the new home is obviously better: nothing in this repository reclaims text automatically, so a copy left behind stays authoritative-looking and then rots — `README.md` § Maintenance Map spent two months describing a button label removed in June, in a duplicate list outside the one section `tests/test_project_map.py` parses. Verify each pointer resolves before handoff; a pointer to a heading that does not exist is worse than the copy it replaced, because it fails silently.
...
- **`master` is production.** Coolify watches that branch and auto-deploys it, so pushing `master` is not "landing code in the repository" — it ships to the gateway 沈予 and 圆圆 are actually living in, and Supabase plus the mounted `/app/data` volume are the real ones. Treat a `master` push as a deployment: complete the verification baseline in `docs/DELIVERY.md` first, and ask the owner before pushing unless they have already said to. Feature branches are not deployed, so an ordinary push to one carries no such weight. A change that reached `master` has passed `deployed`'s trigger but not its evidence: Coolify still has to build and restart, so keep reporting `pushed` until a version endpoint, production log, or build hash confirms the running service.
