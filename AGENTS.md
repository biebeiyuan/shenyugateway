...
For a PWA user-visible fix that is being delivered to production, follow the three non-substitutable acceptance rules in `docs/frontend/STYLE_AND_CRAFT.md` § 交付状态梯与探针边界; that section is the single authority for the delivery status ladder, probe boundaries, and mobile-first acceptance (手机视口约 390px 为第一现场，「是缓存」出口前必须先拿到响应头或产物哈希证据，同一症状不允许用缓存解释第二次).
